...
Rutas públicas principales: `/`, `/login`, `/signup`, `/forgot-password`, `/reset-password?token=...`. Rutas protegidas: `/web/*` (dashboard, gastos, aprobaciones, admin, settings).
...
1. `¿Olvidaste tu contraseña?` → `/forgot-password`.
2. Ingresa email → `POST /auth/forgot-password/`.
3. Backend envía email con link `/reset-password?token=...` (token válido 24h).
4. Accede al link y establece nueva contraseña → `POST /auth/reset-password/`.

### 2.4 Primer acceso y configuración inicial
...
* Desde el avatar/menú: `Logout` → Confirmación modal → (opcional) `POST /auth/logout/`.
...
* **Un usuario no puede aprobar sus propios documentos.** Si un usuario creó o ya intervino en un documento, el sistema evita que ese mismo usuario emita la aprobación final en ese mismo documento.
* **Estados y transiciones:** SAVED → PENDING → (PENDING_L2/PENDING_L3) → APPROVED → PAID. Rechazos devuelven a REJECTED y permiten edición+reenvío.
...
El módulo central de Rinderline permite crear, editar, enviar y gestionar documentos de gasto. Dependiendo del rol del usuario, se muestran funciones específicas (usuario, aprobador o administrador).
...
* Los documentos se guardan inicialmente como borrador hasta que el usuario decida enviarlos.
* Se utiliza *soft delete* para mantener trazabilidad.
...
El sistema realiza la subida al servidor o almacenamiento externo (según configuración), mostrando una barra de progreso.
...
* No existe eliminación permanente; se usa **borrado lógico**.
* Opción **“Archivar”** disponible para documentos sin movimiento o rechazados.
...
### 6.6 Descargar documentos

* Botón para descargar PDF del documento completo o solo los anexos.
* La descarga se realiza mediante el endpoint correspondiente.

### 6.7 Historial del documento
...
* El solicitante recibe notificación automática.
...
* Alertas cuando un documento cambia de estado o requiere acción.

### 8.2 Notificaciones por correo (si aplica)

* Envío de correos en eventos claves: recuperación de contraseña, aprobación, rechazo, reasignación.

### 8.3 Estados que generan notificaciones

* **PENDING** → al enviar a aprobación.
* **REJECTED** → al ser devuelto por el aprobador.
* **APPROVED** → al completar todos los niveles.

---

## 9. Configuración de la Cuenta

### 9.1 Actualizar datos personales

* Desde el menú del usuario, opción **“Mi Perfil”**.
...
### 9.3 Cambiar foto de perfil

* Subir imagen (JPG/PNG) < 2MB.
* Vista previa disponible.

### 9.4 Preferencias de usuario

* Configuración de idioma (ES/EN).
* Preferencia de notificaciones.
* Modo oscuro/claro (si está habilitado).

---

## 10. Reportes y Exportaciones

El módulo de reportes permite generar vistas consolidadas de gastos, documentos y estados. Las opciones disponibles dependen del rol del usuario.

### 10.1 Generar reportes

* Acceder al módulo **Reportes** desde el menú lateral.
* Seleccionar el tipo de reporte: gastos por categoría, documentos por estado, historial de aprobaciones, gastos por centro de costo, etc.
* Establecer filtros: rangos de fecha, estado, tipo de documento, empresa o área.

Los reportes se generan dinámicamente en tablas o gráficos según aplique.

### 10.2 Exportar a Excel o PDF

* Desde el encabezado del reporte, usar el botón **Exportar**.
* Seleccionar formato: **Excel (.xlsx)** o **PDF (.pdf)**.
* La exportación respeta los filtros aplicados.
* Los archivos se descargan directamente al dispositivo.

### 10.3 Filtros avanzados

* Filtros combinados por múltiples columnas.
* Buscador con debounce.
* Filtros por empresa, área y centro de costo.
* Filtros rápidos: “Este mes”, “Últimos 7 días”, “Pendientes”.
...
* Si olvidó su contraseña, utilice la función de recuperación.

### 12.2 No carga un archivo PDF

* Verifique el tamaño (máx. 5MB) y el formato del archivo.
* Revise su conexión a internet.

### 12.3 No se muestra información en la tabla

* Compruebe que los filtros no estén restringiendo la búsqueda.
* Actualice la página.

### 12.4 No puedo aprobar un documento
...
### 13.1 ¿Por qué no me aparecen mis documentos?

* Puede haber filtros activos.
* El documento pudo ser archivado.

### 13.2 ¿Qué significa el estado X?
...
### 13.3 ¿Cómo sé si ya aprobaron mi documento?
...
*Fin de la Parte 2 (Puntos 6–9). La Parte 3 contendrá los puntos 10–14 junto con la propuesta completa de anexos y glosario.*
...
*Fin de la Parte 1 (Puntos 1–5). En la Parte 2 entregaré los puntos 6–9 y en la Parte 3 los puntos 10–14 junto con los anexos visuales y el glosario.*
